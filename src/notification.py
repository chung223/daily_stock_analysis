...
class NotificationChannel(Enum):
    ...
    EMAIL = "email"        # 郵件
    PUSHOVER = "pushover"  # Pushover（手机/桌面推播）
    PUSHPLUS = "pushplus"  # PushPlus（国内推播服务）
    SERVERCHAN3 = "serverchan3"  # Server酱3（手机APP推播服务）
...
class NotificationService(
    ...
    def __init__(self, source_message: Optional[BotMessage] = None):
        ...
        检测所有已設定的渠道，推播时会向所有渠道發送
        """
        config = get_config()
        self._source_message = source_message
        self._context_channels: List[str] = []

        # Markdown 转圖片（Issue #289）
        ...
        # 仅分析結果摘要（Issue #262）：true 时只推播汇总，不含个股详情
        ...
        if not self._available_channels and not self._context_channels:
            logger.warning("未設定有效的通知渠道，将不發送推播通知")
        else:
            channel_names = [ChannelDetector.get_channel_name(ch) for ch in self._available_channels]
            channel_names.extend(self._context_channels)
            logger.info(f"已設定 {len(channel_names)} 个通知渠道：{', '.join(channel_names)}")
    ...
    def _extract_dingtalk_session_webhook(self) -> Optional[str]:
        """从来源訊息中提取钉钉会话 Webhook（用于 Stream 模式回复）"""
    ...
    def send_to_context(self, content: str) -> bool:
        """
        向基于訊息上下文的渠道發送訊息（例如钉钉 Stream 会话）
    ...
    def _send_via_source_context(self, content: str) -> bool:
        """
        使用訊息上下文（如钉钉/飞书会话）發送一份報告
        
        主要用于从機器人 Stream 模式触发的任務，确保结果能回到触发的会话。
        """
        success = False
        ...
        if session_webhook:
            try:
                if self._send_dingtalk_chunked(session_webhook, content, max_bytes=20000):
                    logger.info("已通过钉钉会话（Stream）推播報告")
                    success = True
                else:
                    logger.error("钉钉会话（Stream）推播失敗")
            except Exception as e:
                logger.error(f"钉钉会话（Stream）推播异常: {e}")
        ...
        if feishu_info:
            try:
                if self._send_feishu_stream_reply(feishu_info["chat_id"], content):
                    logger.info("已通过飞书会话（Stream）推播報告")
                    success = True
                else:
                    logger.error("飞书会话（Stream）推播失敗")
            except Exception as e:
                logger.error(f"飞书会话（Stream）推播异常: {e}")

        return success

    def _send_feishu_stream_reply(self, chat_id: str, content: str) -> bool:
        """
        通过飞书 Stream 模式發送訊息到指定会话
        ...
        try:
            ...
            if not FEISHU_SDK_AVAILABLE:
                logger.warning("飞书 SDK 不可用，无法發送 Stream 回复")
                return False
            ...
            if not app_id or not app_secret:
                logger.warning("飞书 APP_ID 或 APP_SECRET 未設定")
                return False
            ...
            logger.error(f"导入飞书 Stream 模块失敗: {e}")
            return False
        except Exception as e:
    ...
    def _send_feishu_stream_chunked(
        ...
        for i, chunk in enumerate(chunks):
            ...
            if not reply_client.send_to_chat(chat_id, chunk):
                success = False
                logger.error(f"飞书 Stream 分块 {i+1}/{len(chunks)} 發送失敗")
    ...
    def generate_daily_report(
        ...
        report_lines = [
            ...
            "",
            f"> 共分析 **{len(results)}** 只股票 | 報告產生時間：{datetime.now().strftime('%H:%M:%S')}",
            "",
            "---",
            "",
        ]
        ...
        report_lines.extend([
            ...
            "| 指标 | 数值 |",
            "|------|------|",
            f"| 🟢 建议買入/加碼 | **{buy_count}** 只 |",
            f"| 🟡 建議持有/觀望 | **{hold_count}** 只 |",
            f"| 🔴 建议減碼/賣出 | **{sell_count}** 只 |",
        ...
        if self._report_summary_only:
            ...
            for result in sorted_results:
                ...
                report_lines.extend([
                    ...
                    "",
                    f"**操作建議：{result.operation_advice}** | **綜合評分：{result.sentiment_score}分** | **趨勢预测：{result.trend_prediction}** | **置信度：{confidence_stars}**",
                    "",
                ])
    ...
    def generate_dashboard_report(
        ...
        # 逐个股票的決策儀表板（Issue #262: summary_only 时跳过详情）
            ...
            for result in sorted_results:
                ...
                if data_persp:
                    ...
                    if trend_data:
                        ...
                        report_lines.extend([
                            f"**均线排列**: {trend_data.get('ma_alignment', 'N/A')} | 多头排列: {is_bullish} | 趨勢强度: {trend_data.get('trend_score', 'N/A')}/100",
                            "",
                        ])
                    # 价格位置
                    if price_data:
                ...
                if battle:
                    ...
                    if sniper:
                        report_lines.extend([
                            ...
                            "| 点位类型 | 价格 |",
                            "|---------|------|",
                            f"| 🎯 理想買入点 | {self._clean_sniper_value(sniper.get('ideal_buy', 'N/A'))} |",
                            f"| 🔵 次优買入点 | {self._clean_sniper_value(sniper.get('secondary_buy', 'N/A'))} |",
                    ...
                    if position:
                        report_lines.extend([
                            f"**💰 倉位建议**: {position.get('suggested_position', 'N/A')}",
    ...
    def generate_wechat_dashboard(self, results: List[AnalysisResult]) -> str:
        ...
        # 底部
        lines.append(f"*產生時間: {datetime.now().strftime('%H:%M')}*")
    ...
    def generate_wechat_summary(self, results: List[AnalysisResult]) -> str:
        ...
        lines.extend([
            "---",
            "*AI 生成，僅供參考，不構成投資建議*",
            f"*详细報告见 reports/report_{report_date.replace('-', '')}.md*"
        ])
    ...
    def generate_single_stock_report(self, result: AnalysisResult) -> str:
        """
        生成单只股票的分析報告（用于单股推播模式 #55）
        
        格式精简但信息完整，适合每分析完一只股票立即推播
        ...
        lines.append("*AI 生成，僅供參考，不構成投資建議*")
    ...
    def _should_use_image_for_channel(
        ...
        if channel == NotificationChannel.WECHAT and len(image_bytes) > WECHAT_IMAGE_MAX_BYTES:
            logger.warning(
                "企业微信圖片超限 (%d bytes)，回退为 Markdown 文本發送",
                len(image_bytes),
            )
            return False
        return True
    ...
    def send(
        ...
        Args:
            ...
            email_stock_codes: 股票代碼列表（可选，用于郵件渠道路由到对应分组邮箱，Issue #268）
            email_send_to_all: 郵件是否发往所有設定邮箱（用于大盤複盤等无股票归属的内容）
        ...
        if not self._available_channels:
            if context_success:
                logger.info("已通过訊息上下文渠道完成推播（无其他通知渠道）")
                return True
            logger.warning("通知服务不可用，跳过推播")
            return False
        ...
        if channels_needing_image:
            ...
            if image_bytes:
                logger.info("Markdown 已转换为圖片，将向 %s 發送圖片",
                ...
                logger.warning(
                    "Markdown 转圖片失敗，将回退为文本發送。请檢查 MARKDOWN_TO_IMAGE_CHANNELS 設定并安装 %s",
                    hint,
                )

        channel_names = self.get_channel_names()
        logger.info(f"正在向 {len(self._available_channels)} 个渠道發送通知：{channel_names}")
        ...
        logger.info(f"通知發送完成：成功 {success_count} 个，失敗 {fail_count} 个")
...
if __name__ == "__main__":
    ...
    if service.is_available():
        print(f"\n=== 推播测试（{service.get_channel_names()}）===")
        success = service.send(report)
        print(f"推播结果: {'成功' if success else '失敗'}")
    else:
        print("\n通知渠道未設定，跳过推播测试")
